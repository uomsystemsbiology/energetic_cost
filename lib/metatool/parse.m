...
%A# parser for Metatool; can read the regular and the extended format
if nargin < 2
...
if any(undeclared)
  ...
  undeclared= undeclared(isnan(str2double(undecl_met))); %A# the other errors will be reported later
...
%     pos= num_met_tok(sub2ind(size(cat_sect), i, lhrh)) %A# pos + j indexes met_ind and ext_met
    ...
    for j= 1:length(syms)
      ...
      if ext_met(pos+j)
        ext(met_ind(pos+j), reac)= coeff;
      else
        st(met_ind(pos+j), reac)= coeff;
      end
